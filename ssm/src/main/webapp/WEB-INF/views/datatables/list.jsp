            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editForm">
                        ...
                        <div class="form-group">
                            <label>书籍名称</label>
                            <input type="text" name="bookname" class="form-control " value="${book.bookname}" id="edit_bookname"/>
                        </div>
                        <div class="form-group">
                            <label>书籍作者</label>
                            <input type="text" name="bookauthor" class="form-control " value="${book.bookauthor}" id="edit_bookauthor"/>
                        </div>
                        <div class="form-group">
                            <label>书籍价格</label>
                            <input type="text" name="bookprice" class="form-control " value="${book.bookprice}" id="edit_bookprice"/>
                        </div>
                        <div class="form-group">
                            <label>书籍数量</label>
                            <input type="text" name="booknum" class="form-control " value="${book.booknum}" id="edit_booknum"/>
                        </div>
                        <div class="form-group">
                            <label>分类</label>
...
<script>
    $(function(){
        var dataTable=$("#dataTable").DataTable({
            "lengthMenu":[5,10,25,50,75,100],//定义每页显示的数据数量
            "serverSide":true,//表示所有的操作都在服务端进行
            "ajax":{
                url:"/datatable/data.json",//服务端URL地址
            ...
            "columns":[//配置返回的JSON中[data]列中数据Key和表格列的对应关系
                {"data":"id","name":"id"},
                {"data":"bookname"},
                {"data":"bookauthor"},
                ...
                {"data":function(row){//row Json对象
                    return "<a href='javascript:;' class='editLink' rel='"+row.id+"'>编辑</a> " +
                            "<a href='javascript:;' class='delLink' rel='"+row.id+"'>删除</a>";
                }}
            ],
            "columnDefs":[//定义列的特征
                ...
                $.post("/datatable/new",$(form).serialize())
                        .done(function(data){
                            if(data=="success"){
                                ...
                                dataTable.ajax.reload();//让dataTables主键自动刷新
                            }
                        })
                        .fail(function(){
                            alert("请求服务器异常");
        ...
        //删除书籍(事件委托机制）可以给当前或未来出现的符合选择器的元素绑定事件
